...
# --- Oracle commands run VERBATIM (override) ---
# check/test tools route through `rtk proxy`: still tracked, but output is NEVER
# summarized. A lossy summary that drops a diagnostic, or a wrapper that masks a
# nonzero exit code, poisons the agent's edit-check loop. Oracle fidelity outranks
# token savings. Runs after the chain so it overrides any lossy wrapper above, and
# also catches oracles the chain misses (pnpm exec tsc, pyright, biome, ...).
if echo "$MATCH_CMD" | grep -qE '^((pnpm|npx)[[:space:]]+(exec[[:space:]]+)?)?(cargo[[:space:]]+(\+[^[:space:]]+[[:space:]]+)?(check|clippy|test|nextest)|nextest[[:space:]]+run|tsc|vue-tsc|eslint|biome|ruff[[:space:]]+check|pytest|mypy|pyright|playwright|vitest|go[[:space:]]+(test|vet)|golangci-lint)([[:space:]]|$)' \
 || echo "$MATCH_CMD" | grep -qE '^(pnpm|npm)[[:space:]]+test([[:space:]]|$)' \
 || echo "$MATCH_CMD" | grep -qE '^python[[:space:]]+-m[[:space:]]+(pytest|mypy)([[:space:]]|$)'; then
